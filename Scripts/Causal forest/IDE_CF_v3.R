...
#[1] "sand_mean"         "mean_sr"           "MAP"               "seasonality_index" "Domcover"            
...
X <- te1%>%
  ungroup()%>%
  dplyr::select( MAP,seasonality_index,mean_sr,Domcover,sand_mean,aridity_index,cv_ppt_inter,PerenGrassCover,n)#,organicmatter)#drtsev.1) #put just the moderators you're testing here
...
varimp <- variable_importance(eval.forest)
ranked.vars <- order(varimp, decreasing = TRUE)
colnames(X)[ranked.vars[1:9]]
#[1] "sand_mean"         "PerenGrassCover"   "mean_sr"           "n"     "seasonality_index" "MAP"               "Domcover"          "aridity_index"     "cv_ppt_inter"   
