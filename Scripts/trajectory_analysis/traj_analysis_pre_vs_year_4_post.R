...
## ------------------------------------------------------- ##
#     PRE-TREATMENT VS YEAR 4 POST-TREATMENT VERSION 
## ------------------------------------------------------- ##
...
export_folder <- paste0("export_trajectories_pre_vs_year_4_post_", Sys.Date())
trajectory_folder <- file.path(export_folder, "pre_vs_year_4_post_trajectory_plots")
summary_stats_folder <- file.path(export_folder, "pre_vs_year_4_post_summary_stats")
...
# Filter our data to year 4 post-treatment
comp_year_4 <- comp_raw %>%
  dplyr::mutate(trt = dplyr::case_when(
    trt == "Control_Infrastructure" ~ "Control",
    TRUE ~ trt)) %>%
  # Filter out unwanted plot types
  dplyr::filter(trt %in% c("Control", "Drought")) %>%
  # Keep only year 4 post-treatment
  dplyr::filter(n_treat_years == 4)

# Sites that have year 4 post-treatment data but not year 0 data
bad_sites_A <- setdiff(x = unique(comp_year_4$site_code), y = unique(comp_pre_treatment$site_code))

# Sites that have year 0 data but not year 4 post-treatment data
bad_sites_B <- setdiff(x = unique(comp_pre_treatment$site_code), y = unique(comp_year_4$site_code))

# Combine both together
all_comp <- bind_rows(comp_pre_treatment, comp_year_4)

# Make sure its only 0 and 4
unique(all_comp$n_treat_years)

# Make a list of "bad" sites that will break the loop
bad_sites <- c(bad_sites_A, bad_sites_B)
...
pdf(file.path(trajectory_folder, "traj_plots_pre_vs_year_4_post.pdf"))
par(mfrow = c(3,3))
...
for (a_site in names(fit_list)){
  ...
  mtext("Trajectory plots for pre-treatment vs. year 4 post-treatment", outer=TRUE,  cex=0.7, line=-1.5)
  
}
dev.off()

# Extract the trajectory analysis statistics for each site except matador.ca
for (a_site in setdiff(x = names(fit_list), y = c("matador.ca"))){
  site_stats <- stat_extract(mod_fit = fit_list[[a_site]])
  stats_list[[a_site]] <- site_stats
}

for (a_bad_site in c("matador.ca")){
  ...
  # Put this site's magnitude distance statistics back into our list
