...
for (a_site in setdiff(x = unique(all_comp$site_code), y = bad_sites)){
  ...
  if(ncol(comp) <= 5){
    ...
    traj_plot <- plot(traj_fit, pch = 3, cex = 0.7, col = "gray", main = paste0(a_site, " post-treatment years only"))
...
pdf(file.path(trajectory_folder, "traj_plots_post_treatment.pdf"))
par(mfrow = c(3,3))
...
for (a_site in names(fit_list)){
  ...
  mtext("Trajectory plots for post-treatment years only", outer=TRUE,  cex=0.7, line=-1.5)
