...
## ------------------------------------------------------- ##
#     PRE-TREATMENT VS YEAR 3 POST-TREATMENT VERSION 
## ------------------------------------------------------- ##
...
summary_stats_folder <- file.path(export_folder, "pre_vs_year_3_post_summary_stats")
...
# Filter our data to year 3 post-treatment
comp_year_3 <- comp_raw %>%
  dplyr::mutate(trt = dplyr::case_when(
    trt == "Control_Infrastructure" ~ "Control",
    TRUE ~ trt)) %>%
  # Filter out unwanted plot types
  dplyr::filter(trt %in% c("Control", "Drought")) %>%
  # Keep only year 3 post-treatment
  dplyr::filter(n_treat_years == 3)

# Sites that have year 3 post-treatment data but not year 0 data
bad_sites_A <- setdiff(x = unique(comp_year_3$site_code), y = unique(comp_pre_treatment$site_code))

# Sites that have year 0 data but not year 3 post-treatment data
bad_sites_B <- setdiff(x = unique(comp_pre_treatment$site_code), y = unique(comp_year_3$site_code))

# Combine both together
all_comp <- bind_rows(comp_pre_treatment, comp_year_3)

# Make sure its only 0 and 3
...
pdf(file.path(trajectory_folder, "traj_plots_pre_vs_year_3_post.pdf"))
par(mfrow = c(3,3))
...
for (a_site in names(fit_list)){
  ...
  mtext("Trajectory plots for pre-treatment vs. year 3 post-treatment", outer=TRUE,  cex=0.7, line=-1.5)
...
for (a_bad_site in c("allmendo.ch", "cobar.au", "ebro.es", "esw.ca", "kernnu.ca")){
