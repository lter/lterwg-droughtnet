...
data.anpp <- read.csv("C:/Users/ohler/Dropbox/IDE MS_Single year extreme/Data/full_biomass_03-23-2026.csv")%>%
              dplyr::select(site_code, year, n_treat_days)%>%
              unique()
data.anpp$n_treat_days <- as.numeric(data.anpp$n_treat_days)

data.cover <- read.csv("C:/Users/ohler/Dropbox/IDE/data_raw/full_cover_2026-03-25.csv")%>%
              dplyr::select(site_code, year, n_treat_days)%>%
              unique()%>%
...
write.csv(combfin, "C:/Users/ohler/Dropbox/IDE/data_processed/IDE_treatment_years_2026-03-25.csv")
