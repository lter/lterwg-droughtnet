...
theme_set(theme_bw(20)) # set theme
...
datblip<-dat%>% # prep dataframe for looping
  mutate(rep=paste(site_code, replicate, sep=";")) %>% 
  # standardizing lifeform and lifespan information
  mutate(local_lifeform = ifelse(local_lifeform %in% c("GRAMINOID","Grass"),"GRASS",
                                 ifelse(local_lifeform %in% c("VINE","TREE","WOODY","SHRUB","SUBSHRUB"),"WOODY",
                                        ifelse(local_lifeform %in% c("BRYOPHYTE","MOSS","CLUBMOSS","LICHEN","FUNGI"),"NONVASCULAR",local_lifeform))),
         local_lifespan = ifelse(local_lifespan == "BIENNIAL","PERENNIAL",
...
sc<-unique(datblip$site_code) # site codes for looping
